...
HMI** de `ClaudeHMI-FW` en un preview de 480×320 (misma resolución que el ST7796 del dispositivo).

- No toca tu simulador original (`lv_port_pc_visual_studio`) ni su otro proyecto.
- Compila **las fuentes reales** de `..\ClaudeHMI-FW\main\ui` (no copias), así que lo que ves aquí es lo
...
| `LvglWindowsSimulator.vcxproj` | Include dirs → `ClaudeHMI-FW\main\ui` + `sim_app`; lista de fuentes de la HMI |
...
de `ClaudeHMI-FW\main\ui`, recompila el sim para ver el cambio; el mismo código va a la placa.
